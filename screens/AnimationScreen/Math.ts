...
export class BubbleBlob {
  ...
  generateCoords() {
    ...
    let pathD = "";
    let pathCoordinates = [];
    // random number between 120 and 180
    let radius = Math.floor(Math.random() * 60) + 120;

    // random number between 0 and 100
    let centerX = 200;
    let centerY = 200;
    // random number between 0.1 and 0.9

    let vertixCountFactor = 0.2;
...
export function getCoordinates() {
  const blob = new BubbleBlob();

  blob.generateCoords();

  return blob.pathCoordinates;
}
